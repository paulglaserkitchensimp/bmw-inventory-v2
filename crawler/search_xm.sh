...
# xm_{YEARS}_results.json. Use --sync to UNION into results.json (never
# replaces existing inventory) and refresh the visualizer.
#
# Model designations per source (verified 2026-07):
#   search_inventory : XM (no trim — covers base + Label/Label Red)
#   Autotrader path  : bmw/xm
#   Cars.com         : models[]=bmw-xm
#   TrueCar mmt[]    : bmw_xm   (used listings only — new XMs come from dealers)
#
# NOTE: min-miles defaults to 0 here (unlike the used-deal sweeps' 60) so
# brand-new delivery-mileage XMs are included. Pass --min-miles 60 to restrict
# to used/loaner cars.
#
# Usage:
#   ./search_xm.sh                     # 2026 (default)
#   ./search_xm.sh --year 2025-2026 --sync
#   ./search_xm.sh --skip-fetch --skip-vdp --headless
...
usage() {
  ...
  echo "  --out FILE          Merged output path (default: xm_{YEARS}_results.json)"
...
echo "═══════════════════════════════════════════════════════════════"
echo " Targeted search: BMW XM (${YEAR})"
...
if ! $SKIP_AUTOTRADER; then
  echo
  echo "▶ [2/4] Autotrader — bmw/xm"
  at_cmd=(
    uv run crawl_autotrader.py
    --max-pages 30
    --url "https://www.autotrader.com/cars-for-sale/all-cars/bmw/xm/detroit-mi?mileage=${MAX_MILES}&searchRadius=0&startYear=${YEAR_LOW}&endYear=${YEAR_HIGH}"
    --min-miles "$MIN_MILES"
    --max-miles "$MAX_MILES"
    --out "$AT_OUT"
  )
...
if ! $SKIP_CARS_COM; then
  echo
  echo "▶ [3/4] Cars.com — bmw-xm"
  cars_cmd=(
    uv run cars_com.py
    --url "https://www.cars.com/shopping/results/?mileage_min=${MIN_MILES}&mileage_max=${MAX_MILES}&zip=48226&maximum_distance=9999&year_min=${YEAR_LOW}&year_max=${YEAR_HIGH}&makes[]=bmw&models[]=bmw-xm&sort=best_match_desc"
    --min-miles "$MIN_MILES"
    --max-miles "$MAX_MILES"
    --out "$CARS_OUT"
...
if ! $SKIP_TRUECAR; then
  echo
  echo "▶ [4/4] TrueCar — mmt[]=bmw_xm"
  uv run truecar.py \
    --url "https://www.truecar.com/used-cars-for-sale/listings/inventory/?searchRadius=5000&yearLow=${YEAR_LOW}&yearHigh=${YEAR_HIGH}&mileageHigh=${MAX_MILES}&mmt[]=bmw_xm" \
    --min-miles "$MIN_MILES" \
    --max-miles "$MAX_MILES" \
    --out "$TC_OUT" \
    --exclude-states "$EXCLUDE_STATES"
