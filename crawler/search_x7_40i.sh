#!/usr/bin/env bash
# Targeted nationwide sweep — BMW X7 xDrive40i, <15k miles.
#
...
# x7_40i_{YEARS}_results.json. Use --sync to UNION into results.json (never
# replaces existing inventory) and refresh the visualizer.
#
# Model/trim designations per source (verified 2026-07):
#   search_inventory : X7:xDrive40i
#   Autotrader path  : bmw/x7/xdrive40i
#   Cars.com         : models[]=bmw-x7&trims[]=bmw-x7-xdrive40i
#   TrueCar mmt[]    : bmw_x7_40i   (TrueCar calls the trim just "40i")
#
# Usage:
#   ./search_x7_40i.sh                     # 2025-2026 (default)
#   ./search_x7_40i.sh --year 2024-2026 --sync
#   ./search_x7_40i.sh --skip-fetch --skip-vdp --headless
...
usage() {
  ...
  echo "  --out FILE          Merged output path (default: x7_40i_{YEARS}_results.json)"
...
echo "═══════════════════════════════════════════════════════════════"
echo " Targeted search: X7 xDrive40i (${YEAR})"
...
if ! $SKIP_AUTOTRADER; then
  echo
  echo "▶ [2/4] Autotrader — x7/xdrive40i"
  at_cmd=(
    uv run crawl_autotrader.py
    --max-pages 30
    --url "https://www.autotrader.com/cars-for-sale/all-cars/bmw/x7/xdrive40i/detroit-mi?mileage=${MAX_MILES}&searchRadius=0&startYear=${YEAR_LOW}&endYear=${YEAR_HIGH}"
    --min-miles "$MIN_MILES"
    --max-miles "$MAX_MILES"
    --out "$AT_OUT"
  )
...
if ! $SKIP_CARS_COM; then
  echo
  echo "▶ [3/4] Cars.com — bmw-x7 / bmw-x7-xdrive40i"
  cars_cmd=(
    uv run cars_com.py
    --url "https://www.cars.com/shopping/results/?mileage_min=${MIN_MILES}&mileage_max=${MAX_MILES}&zip=48226&maximum_distance=9999&year_min=${YEAR_LOW}&year_max=${YEAR_HIGH}&makes[]=bmw&models[]=bmw-x7&trims[]=bmw-x7-xdrive40i&sort=best_match_desc"
    --min-miles "$MIN_MILES"
    --max-miles "$MAX_MILES"
    --out "$CARS_OUT"
...
if ! $SKIP_TRUECAR; then
  echo
  echo "▶ [4/4] TrueCar — mmt[]=bmw_x7_40i"
  uv run truecar.py \
    --url "https://www.truecar.com/used-cars-for-sale/listings/inventory/?searchRadius=5000&yearLow=${YEAR_LOW}&yearHigh=${YEAR_HIGH}&mileageHigh=${MAX_MILES}&mmt[]=bmw_x7_40i" \
    --min-miles "$MIN_MILES" \
    --max-miles "$MAX_MILES" \
    --out "$TC_OUT" \
    --exclude-states "$EXCLUDE_STATES"
